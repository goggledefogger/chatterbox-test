...
![Chatterbox Turbo Header](./Chatterbox-Turbo.jpg)

**Chatterbox Studio** is a high-performance, developer-friendly Streamlit interface for state-of-the-art open-source Text-to-Speech models by **Resemble AI**.
...
- **Granular Progress Tracking**: Real-time feedback during model loading and audio synthesis.
- **Hugging Face Integration**: Secure token input with real-time download progress bars.
...
```bash
uv pip install -e .
uv pip install streamlit streamlit-extras
```
...
- **Architecture**: The app uses a robust, thread-safe progress tracking system (`StreamlitTqdm`) to handle Hugging Face's concurrent download process.
...
Built with ❤️ by [Resemble AI](https://resemble.ai)
